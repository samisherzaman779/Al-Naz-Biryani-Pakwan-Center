...
type NavSection = "Home" | "Menu" | "About" | "Contact";

type AlNazWebsiteProps = {
...
function Toast({ toasts }: { toasts: ToastItem[] }) {
  return (
    <div style={{ position: "fixed", bottom: "7rem", left: "50%", transform: "translateX(-50%)", zIndex: 9999, display: "flex", flexDirection: "column", gap: "8px", pointerEvents: "none", alignItems: "center" }}>
      {toasts.map((toast) => (
        <div key={toast.id} style={{ background: "#d4a017", color: "#0a0704", padding: "10px 22px", fontSize: "0.82rem", fontWeight: "bold", letterSpacing: "0.05em", animation: "toastIn .3s ease", whiteSpace: "nowrap", boxShadow: "0 4px 24px rgba(0,0,0,.5)" }}>
          {toast.msg}
        </div>
      ))}
    </div>
...
function CheckoutModal({
  ...
  const validate = () => {
    ...
    if (!/^[\d\s\+\-]{10,14}$/.test(form.phone.trim())) nextErrors.phone = "Valid phone number required";
    if (orderType === "delivery" && !form.address.trim()) nextErrors.address = "Delivery address required";
  ...
  const inp = (error?: string) => ({
    ...
    border: `1px solid ${error ? "#c47a2a" : "rgba(212,160,23,.2)"}`,
    color: "#f5ead8",
    padding: "11px 13px",
    fontSize: "0.88rem",
    outline: "none",
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(540px,96vw)", maxHeight: "92vh", overflowY: "auto", background: "#100c07", border: "1px solid rgba(212,160,23,.2)", zIndex: 301, animation: "scaleIn .25s ease" }}>
        {step < 3 && (
          <div style={{ display: "flex", borderBottom: "1px solid rgba(212,160,23,.08)" }}>
            {["Details", "Review", "Done"].map((stepName, index) => (
              <div key={stepName} style={{ flex: 1, padding: "0.9rem", textAlign: "center", borderBottom: step === index + 1 ? "2px solid #d4a017" : "2px solid transparent", background: step === index + 1 ? "rgba(212,160,23,.05)" : "transparent" }}>
                <span style={{ fontSize: "0.65rem", letterSpacing: ".18em", textTransform: "uppercase", color: step >= index + 1 ? "#d4a017" : "#4a3e30" }}>
                  {step > index + 1 ? "✓ " : ""}
                  {stepName}
                </span>
              </div>
        ...
        <div style={{ padding: "1.8rem" }}>
            ...
            <>
              ...
              <div style={{ marginBottom: "1.6rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.7rem" }}>
                  {[
                    ["delivery", "🛵", "Delivery", `+Rs.${DELIVERY_FEE}`],
                    ["pickup", "🏃", "Pickup", "Free"],
                  ].map(([value, icon, label, sub]) => (
                    <div key={value} onClick={() => setOrderType(value as "delivery" | "pickup")} style={{ padding: "0.9rem", border: `1px solid ${orderType === value ? "#d4a017" : "rgba(212,160,23,.15)"}`, background: orderType === value ? "rgba(212,160,23,.07)" : "transparent", cursor: "pointer", textAlign: "center", transition: "all .2s" }}>
                      <div style={{ fontSize: "1.5rem", marginBottom: "3px" }}>{icon}</div>
                      <div style={{ fontSize: "0.88rem", color: orderType === value ? "#d4a017" : "#f5ead8", fontWeight: "bold" }}>{label}</div>
              ...
              <div style={{ marginBottom: "1.1rem" }}>
                ...
                <input style={inp(errors.name)} placeholder="Aapka naam" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
                {errors.name && <p style={{ color: "#c47a2a", fontSize: "0.72rem", marginTop: "4px" }}>{errors.name}</p>}
              </div>

              <div style={{ marginBottom: "1.1rem" }}>
                <label style={labelStyle}>Phone / WhatsApp *</label>
                <input style={inp(errors.phone)} placeholder="03XX-XXXXXXX" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} />
              ...
              {orderType === "delivery" && (
                <div style={{ marginBottom: "1.1rem" }}>
                  ...
                  <textarea style={{ ...inp(errors.address), resize: "none", height: "76px" }} placeholder="Ghar ka pura address..." value={form.address} onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))} />
              ...
              <div style={{ marginBottom: "1.6rem" }}>
                ...
                <textarea style={{ ...inp(), resize: "none", height: "64px" }} placeholder="Extra spicy, less oil, etc." value={form.note} onChange={(e) => setForm((f) => ({ ...f, note: e.target.value }))} />
              </div>
          ...
          {step === 2 && (
            <>
              ...
              <div style={{ background: "rgba(212,160,23,.04)", border: "1px solid rgba(212,160,23,.1)", padding: "1rem", marginBottom: "1.3rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.7rem" }}>
                  {[["👤 Name", form.name], ["📞 Phone", form.phone], ["📦 Type", orderType === "delivery" ? "🛵 Delivery" : "🏃 Pickup"], ...(orderType === "delivery" ? [["📍 Address", form.address]] : [])].map(([label, value]) => (
                    <div key={label} style={{ gridColumn: label.includes("Address") ? "1/-1" : "auto" }}>
                      <p style={{ fontSize: "0.65rem", color: "#7a6a55", letterSpacing: ".12em", marginBottom: "2px" }}>{label}</p>
                      <p style={{ fontSize: "0.87rem", color: "#f5ead8" }}>{value}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: "1.3rem" }}>
                ...
                {cart.map((item) => (
                  <div key={item.id} style={{ display: "flex", justifyContent: "space-between", padding: "0.6rem 0", borderBottom: "1px solid rgba(212,160,23,.06)" }}>
                    <span style={{ fontSize: "0.86rem", color: "#f5ead8" }}>
                      {item.name} <span style={{ color: "#7a6a55" }}>×{item.qty}</span>
                    </span>
          ...
          {step === 3 && (
            <div style={{ textAlign: "center", padding: "0.5rem 0" }}>
              ...
              <p style={{ color: "#7a6a55", fontSize: "0.88rem", lineHeight: 1.8, marginBottom: "0.4rem" }}>Aapka order WhatsApp per Al-Naz ko bhej diya gaya.</p>
              <p style={{ color: "#a0896a", fontSize: "0.86rem", lineHeight: 1.8, marginBottom: "1.8rem" }}>
                📞 <strong style={{ color: "#f5ead8" }}>+92 345-2958883</strong> per confirm zaroor karen.
                <br />
              ...
              <div style={{ background: "rgba(212,160,23,.05)", border: "1px solid rgba(212,160,23,.1)", padding: "1.2rem", marginBottom: "1.8rem" }}>
                ...
                <p style={{ fontSize: "0.7rem", color: "#4a3e30", marginTop: "4px" }}>{orderType === "delivery" ? `Delivery to: ${form.address}` : "Pickup / Takeaway"}</p>
              </div>
              <button onClick={onClose} style={{ background: "#d4a017", border: "none", color: "#0a0704", padding: "13px 36px", fontSize: "0.8rem", letterSpacing: ".12em", textTransform: "uppercase", cursor: "pointer", fontWeight: "bold" }}>Done 🎉</button>
            </div>
          )}
        </div>
      </div>
...
export default function AlNazWebsite({ initialSection = "Home" }: AlNazWebsiteProps) {
  ...
  useEffect(() => {
    if (initialSection !== "Home") {
      setTimeout(() => scrollTo(initialSection), 0);
    }
  ...
  const addToast = useCallback((msg: string) => {
    ...
    setTimeout(() => setToasts((prev) => prev.filter((item) => item.id !== id)), 2200);
  }, []);
  ...
  const removeItem = useCallback((id: string) => setCart((prev) => prev.filter((item) => item.id !== id)), []);
  const cartCount = useMemo(() => cart.reduce((sum, item) => sum + item.qty, 0), [cart]);
  const cartTotal = useMemo(() => cart.reduce((sum, item) => sum + item.price * item.qty, 0), [cart]);
  const currentItems = useMemo(() => MENU.find((m) => m.category === activeCategory)?.items ?? [], [activeCategory]);
    ...
    <div style={{ fontFamily: "Georgia,'Times New Roman',serif", background: dark, color: text, minHeight: "100vh", overflowX: "hidden" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "0 1.8rem", height: "68px", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(10,7,4,.97)" : "transparent", borderBottom: scrolled ? "1px solid rgba(212,160,23,.15)" : "none", transition: "all .4s", backdropFilter: scrolled ? "blur(12px)" : "none" }}>
        ...
        <ul style={{ display: "flex", gap: "2rem", listStyle: "none", margin: 0, padding: 0 }} className="ndl">
          {(["Home", "Menu", "About", "Contact"] as NavSection[]).map((link) => (
            <li key={link} className="nav-lnk" onClick={() => scrollTo(link)} style={{ cursor: "pointer", fontSize: "0.76rem", letterSpacing: ".14em", textTransform: "uppercase", color: activeNav === link ? gold : "#c8b89a", borderBottom: activeNav === link ? `1px solid ${gold}` : "1px solid transparent", paddingBottom: "2px", transition: "color .2s" }}>
              {link}
            </li>
          ))}
        </ul>
        <button onClick={() => setCartOpen(true)} style={{ background: "transparent", border: `1px solid ${cartCount > 0 ? gold : "rgba(212,160,23,.3)"}`, color: gold, padding: "8px 16px", cursor: "pointer", fontSize: "0.78rem", letterSpacing: ".08em", display: "flex", alignItems: "center", gap: "7px", transition: "all .2s", animation: cartCount > 0 ? "fabPop .4s ease" : "none" }} className="bo">
          ...
          {cartCount > 0 ? (
            <>
              <span style={{ background: gold, color: dark, borderRadius: "50%", width: "19px", height: "19px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.68rem", fontWeight: "bold" }}>{cartCount}</span>
      ...
      <section id="home" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", background: `linear-gradient(135deg,${dark} 0%,#1a0f05 40%,#0f0a04 100%)`, overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, maxWidth: "1200px", margin: "0 auto", padding: "0 1.8rem", paddingTop: "68px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "center", width: "100%" }} className="hi">
          <div>
            <span className="a1" style={{ display: "inline-block", fontSize: "0.68rem", letterSpacing: ".28em", textTransform: "uppercase", color: gold, borderTop: `1px solid ${gold}`, borderBottom: `1px solid ${gold}`, padding: "5px 14px", marginBottom: "1.5rem" }}>Est. Karachi, Pakistan</span>
            ...
            <div className="a4" style={{ marginTop: "2rem", display: "flex", gap: "0.8rem", flexWrap: "wrap" }}>
              <button className="bg" onClick={() => scrollTo("Menu")} style={{ background: gold, color: dark, border: "none", padding: "12px 26px", fontSize: "0.78rem", letterSpacing: ".14em", textTransform: "uppercase", cursor: "pointer", fontWeight: "bold", transition: "all .2s" }}>Order Now 🍛</button>
              <button className="bo" onClick={() => scrollTo("Contact")} style={{ background: "transparent", color: gold, border: `1px solid ${gold}`, padding: "12px 26px", fontSize: "0.78rem", letterSpacing: ".14em", textTransform: "uppercase", cursor: "pointer", transition: "all .2s" }}>Find Us 📍</button>
            </div>
          </div>
                ...
                <div key={item.id} className="mc" style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(212,160,23,.08)", padding: "1.4rem", position: "relative", display: "flex", flexDirection: "column", gap: "0.5rem", transition: "all .2s" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "0.3rem" }}>
                    ...
                    {inCart ? (
                      ...
                      <button className="bg" onClick={() => addToCart(item)} style={{ background: gold, border: "none", color: dark, padding: "7px 15px", fontSize: "0.7rem", letterSpacing: ".12em", textTransform: "uppercase", cursor: "pointer", fontWeight: "bold", transition: "all .2s" }}>+ Add</button>
                    )}
                  </div>
                </div>
              );
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "4.5rem 1.8rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }} className="ag">
            ...
            <div style={{ background: "rgba(212,160,23,.04)", border: "1px solid rgba(212,160,23,.1)", padding: "2.2rem", textAlign: "center", position: "relative" }}>
              ...
              <p style={{ fontStyle: "italic", fontSize: "1.35rem", color: gold, lineHeight: 1.45 }}>
                {"\"Biryani isn't just food."}
                <br />
                {"It's an emotion.\""}
              </p>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "4.5rem 1.8rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem" }} className="cg">
            <div style={{ display: "flex", flexDirection: "column", gap: "1.6rem" }}>
              {[["📍", "Address", "Mehmoodabad, Karachi\nSindh, Pakistan"], ["📞", "Phone / WhatsApp", "+92 345-2958883\n0321-8760608"], ["📧", "Email", "Mehmoodkhalid615@gmail.com"], ["🕒", "Hours", "Monday – Sunday · 3:00 PM – 10:00 PM"], ["🛵", "Delivery", "Available · Foodpanda & Direct Order"]].map(([icon, label, value]) => (
                <div key={label} style={{ display: "flex", gap: "1rem" }}>
                  ...
                  <div>
                    ...
                    {value.split("\n").map((line) => (
                      <p key={line} style={{ fontSize: "0.88rem", color: text, lineHeight: 1.6 }}>{line}</p>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div>
              <div style={{ background: "rgba(212,160,23,.04)", border: "1px solid rgba(212,160,23,.12)", height: "270px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "0.9rem" }}>
                ...
                <p style={{ textAlign: "center", color: "#a0896a", fontSize: "0.86rem", lineHeight: 1.7, padding: "0 1.5rem" }}>
                  <strong style={{ color: gold }}>Al-Naz Biryani & Pakwan Center</strong>
                  <br />
                  Mehmoodabad, Karachi
                  <br />
                  24.8525° N, 67.0655° E
                </p>
                <button className="bo" onClick={() => window.open("https://maps.google.com/?q=Al-Naz+Biryani+Pakwan+Center+Mehmoodabad+Karachi")} style={{ background: "transparent", border: `1px solid ${gold}`, color: gold, padding: "9px 20px", fontSize: "0.74rem", letterSpacing: ".12em", textTransform: "uppercase", cursor: "pointer", transition: "all .2s" }}>Open in Google Maps →</button>
              </div>
